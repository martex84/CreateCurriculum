...
import {
  ...
  ValueInputAtual,
} from "@/interface/createPdfFile";
import {
  realizarCopiaProfunda,
  salvarValorEmObjeto,
  tratarValueInput,
...
function Dados(props: Readonly<Props>) {
  ...
  function realizarCopiaInternaObjectValues(): DadosInterfaceInput {
    let objetoInterno: ObjectViewComponentes = {
      ...templateObjectViewComponentes,
    };
  ...
  function salvarDadosInputAtual(idCampo: string) {
    let copiaObjeto = realizarCopiaProfunda(
      props.getDadosSalvos,
    ) as ObjectValues;

    if (copiaObjeto.ValueInputAtual === undefined)
      copiaObjeto.ValueInputAtual = { ...templateValueInputAtual };

    if (copiaObjeto.objectParameter === undefined)
      copiaObjeto.objectParameter = { ...templateObjectParameter };
    ...
    if (nomeInputSalvo === undefined || nomeInputSalvo !== idCampo) {
      try {
        ...
        let copiaId = realizarCopiaProfunda(templateIdCampos) as Record<
          string,
          any
        >;
        ...
        if (nomeInputSalvo && nomeInputSalvo !== idCampo) {
          ...
          if (
            !salvarValorEmObjeto(
              copiaObjeto.objectParameter,
              nomeCampo,
              copiaObjeto.ValueInputAtual.valorAnteriorInput,
            )
          ) {
            if (
              !salvarValorEmObjeto(
                copiaObjeto.objectParameter.contato,
                nomeCampo,
                copiaObjeto.ValueInputAtual.valorAnteriorInput,
              )
            )
              throw new Error("Falha ao tentar salvar os dados do campo");
          }
  ...
  function inverterValoresView(idCampos: string, salvarDadosInput?: boolean) {
    ...
    switch (idCampos) {
      case templateIdCampos.certificacoes:
        objetoInterno.certificacoes =
          !copiaDadosSalvos.objectViewComponentes?.certificacoes;
        valorCaptado = copiaDadosSalvos.objectParameter?.certificacoes;

        break;
      case templateIdCampos.competencias:
        objetoInterno.competencias =
          !copiaDadosSalvos.objectViewComponentes?.competencias;
        valorCaptado = copiaDadosSalvos.objectParameter?.competencias;

        break;
      case templateIdCampos.formacaoAcademica:
        objetoInterno.formacaoAcademica =
          !copiaDadosSalvos.objectViewComponentes?.formacaoAcademica;
        valorCaptado = copiaDadosSalvos.objectParameter?.formacaoAcademica;

        break;
      case templateIdCampos.historicoProfissinal:
        objetoInterno.historicoProfissional =
          !copiaDadosSalvos.objectViewComponentes?.historicoProfissional;
        valorCaptado = copiaDadosSalvos.objectParameter?.historicoProfissional;

        break;
      case templateIdCampos.idiomas:
        objetoInterno.idiomas =
          !copiaDadosSalvos.objectViewComponentes?.idiomas;
        valorCaptado = copiaDadosSalvos.objectParameter?.idiomas;

        break;
      case templateIdCampos.resumoProfissional:
        objetoInterno.resumoProfissional =
          !copiaDadosSalvos.objectViewComponentes?.resumoProfissional;
      ...
      case templateIdCampos.numero:
        if (objetoInterno.contato) {
          objetoInterno.contato.numero =
            !copiaDadosSalvos.objectViewComponentes?.contato?.numero;
          valorCaptado = copiaDadosSalvos.objectParameter?.contato.numero;
        }
        break;
      case templateIdCampos.email:
        if (objetoInterno.contato) {
          objetoInterno.contato.email =
            !copiaDadosSalvos.objectViewComponentes?.contato?.email;
          valorCaptado = copiaDadosSalvos.objectParameter?.contato.email;
        }
        break;
      case templateIdCampos.linkdin:
        if (objetoInterno.contato) {
          objetoInterno.contato.linkdin =
            !copiaDadosSalvos.objectViewComponentes?.contato?.linkdin;
  ...
  function salvaValores(
    ...
    switch (idCampos) {
      ...
      case templateIdCampos.competencias:
        objetoInterno.competencias = valor;
        break;
      ...
      case templateIdCampos.historicoProfissinal:
        objetoInterno.historicoProfissional = valor;
        break;
  ...
  function escolheCampo() {
        ...
        camposRetorno.push(
          <div key={0}>
            ...
            >
              <div className="flex flex-col gap-y-4">
                <div>
                  <ToolsFormat
                    ...
                    editando={
                      props.getDadosSalvos.objectViewComponentes?.contato
                        ?.numero
                    }
                  ...
                  >
                    {props.getDadosSalvos.objectViewComponentes?.contato
                      ...
                      <input
                        ...
                        value={tratarValueInput(
                          props.getDadosSalvos?.objectParameter?.contato
                            ?.numero,
                        )}
                        onChange={(event) => {
                          salvaValores(
                            templateIdCampos.numero,
                            event.target.value,
                          );
                        }}
                        onClick={() =>
                          salvarDadosInputAtual(templateIdCampos.numero)
                        }
                        ref={camposRef.inputTelefone}
                      />
                    ) : (
                      <div className="containerViewText w-full min-h-6 max-h-6">
                        {HTMLReactParser(
                          convertGenerateComponents(
                            tratarValueInput(
                              props.getDadosSalvos?.objectParameter?.contato
                                ?.numero,
                            ),
                  ...
                  >
                    {props.getDadosSalvos.objectViewComponentes?.contato
                      ...
                      <input
                        ...
                        className="h-6 w-full text-left"
                        value={tratarValueInput(
                          props.getDadosSalvos?.objectParameter?.contato?.email,
                        )}
                        onChange={(event) => {
                          salvaValores(
                            templateIdCampos.email,
                            event.target.value,
                          );
                        }}
                        onClick={() =>
                          salvarDadosInputAtual(templateIdCampos.email)
                        }
                  ...
                  >
                    {props.getDadosSalvos.objectViewComponentes?.contato
                      ...
                      <input
                        type="text"
                        id={templateIdCampos.linkdin}
                        className="w-full text-left min-h-6 max-h-6"
                        value={tratarValueInput(
                          props.getDadosSalvos?.objectParameter?.contato
                            ?.linkdin,
                        )}
                        onChange={(event) => {
                          salvaValores(
                            templateIdCampos.linkdin,
                            event.target.value,
                          );
                        }}
                        onClick={() =>
                          salvarDadosInputAtual(templateIdCampos.linkdin)
                        }
              ...
              <div className="flex flex-col">
                ...
                <button
                  className="size-full"
                  onClick={() => {
                    if (!props.getDadosSalvos.objectViewComponentes?.nome)
                      inverterValoresView(templateIdCampos.nome);
                  }}
                >
                  {props.getDadosSalvos?.objectViewComponentes?.nome ? (
                    <input
                      ...
                      className="w-full text-left min-h-6 max-h-6"
                      value={tratarValueInput(
                        props.getDadosSalvos?.objectParameter?.nome,
                      )}
                      onChange={(event) => {
                        salvaValores(templateIdCampos.nome, event.target.value);
                      }}
                      onClick={() =>
                        salvarDadosInputAtual(templateIdCampos.nome)
                      }
              ...
              <div className="flex flex-col">
                ...
                <button
                  className="size-full"
                  onClick={() => {
                    if (!props.getDadosSalvos.objectViewComponentes?.idiomas)
                      inverterValoresView(templateIdCampos.idiomas);
                  }}
                >
                  {props.getDadosSalvos?.objectViewComponentes?.idiomas ? (
                    <input
                      ...
                      className="w-full text-left min-h-6 max-h-6"
                      value={tratarValueInput(
                        props.getDadosSalvos?.objectParameter?.idiomas,
                      )}
                      onChange={(event) => {
                        salvaValores(
                          templateIdCampos.idiomas,
                          event.target.value,
                        );
                      }}
                      onClick={() =>
                        salvarDadosInputAtual(templateIdCampos.idiomas)
                      }
                      ref={camposRef.inputIdiomas}
                    />
                  ) : (
                    <div className="containerViewText min-h-6 max-h-6">
                      {HTMLReactParser(
                        convertGenerateComponents(
                          tratarValueInput(
                            props.getDadosSalvos?.objectParameter?.idiomas,
            ...
            <div
              className="flex-col gap-y-4"
              style={
                props.parteAtualPreenchimento === 2
                  ? { display: "flex" }
                  : { display: "none" }
              }
            >
              <div className="flex flex-col">
                <ToolsFormat
                  ...
                  functionPrincipal={salvaValores}
                  editando={
                    props.getDadosSalvos.objectViewComponentes
                      ?.resumoProfissional
                  }
                  functionAlterarView={inverterValoresView}
                  visibilidadeCampos={{
                ...
                />
                <label htmlFor={templateIdCampos.resumoProfissional}>
                  Resumo Profissional
                </label>
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      inverterValoresView(templateIdCampos.resumoProfissional);
                  }}
                >
                  {props.getDadosSalvos?.objectViewComponentes
                    ?.resumoProfissional ? (
                    <textarea
                      id={templateIdCampos.resumoProfissional}
                      className="resize-none w-full text-left min-h-24 max-h-24"
                      value={tratarValueInput(
                        props.getDadosSalvos?.objectParameter
                          ?.resumoProfissional,
                      )}
                      onChange={(event) => {
                        salvaValores(
                          templateIdCampos.resumoProfissional,
                          event.target.value,
                        );
                      }}
                      onClick={() =>
                        salvarDadosInputAtual(
                          templateIdCampos.resumoProfissional,
                        )
                      }
                    ...
                    <div className="containerViewText min-h-24 max-h-24">
                      {HTMLReactParser(
                        convertGenerateComponents(
                          tratarValueInput(
                            props.getDadosSalvos?.objectParameter
              ...
              <div className="flex flex-col">
                ...
                />
                <label htmlFor={templateIdCampos.formacaoAcademica}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      inverterValoresView(templateIdCampos.formacaoAcademica);
                  }}
                >
                  {props.getDadosSalvos?.objectViewComponentes
                    ?.formacaoAcademica ? (
                    <textarea
                      ...
                      onChange={(event) => {
                        salvaValores(
                          templateIdCampos.formacaoAcademica,
                          event.target.value,
                        );
                      }}
                      onClick={() =>
              ...
              <div className="flex flex-col">
                <ToolsFormat
                  ...
                  functionPrincipal={salvaValores}
                  editando={
                    props.getDadosSalvos.objectViewComponentes?.competencias
                  }
                  functionAlterarView={inverterValoresView}
                  visibilidadeCampos={{
                    listaOrdenada: true,
                ...
                />
                <label htmlFor={templateIdCampos.competencias}>
                  Competências
                </label>
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      ...
                      inverterValoresView(templateIdCampos.competencias);
                  }}
                >
                  {props.getDadosSalvos?.objectViewComponentes?.competencias ? (
                    <textarea
                      ...
                      className="resize-none w-full text-left min-h-24 max-h-24"
                      value={tratarValueInput(
                        props.getDadosSalvos?.objectParameter?.competencias,
                      )}
                      onChange={(event) => {
                        salvaValores(
                          templateIdCampos.competencias,
                          event.target.value,
                        );
                      }}
                      onClick={() =>
                        salvarDadosInputAtual(templateIdCampos.competencias)
                      }
                      ref={camposRef.textAreaResumoProfissional}
                    />
                  ) : (
            ...
            >
              <div className="flex flex-col">
                ...
                />
                <label htmlFor={templateIdCampos.historicoProfissinal}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        ...
                        inverterValoresView(
                          templateIdCampos.historicoProfissinal,
                        );
                    }}
                  >
                    {props.getDadosSalvos?.objectViewComponentes
                      ...
                      <textarea
                        id={templateIdCampos.historicoProfissinal}
                        className="size-full resize-none w-full text-left max-h-48 min-h-48"
                        value={tratarValueInput(
                          props.getDadosSalvos?.objectParameter
                            ?.historicoProfissional,
                        )}
                      ...
                      <div className="containerViewText max-h-48 min-h-48">
                        {HTMLReactParser(
                          convertGenerateComponents(
                            tratarValueInput(
                              props.getDadosSalvos?.objectParameter
              ...
              <div className="flex flex-col">
                ...
                />
                <label htmlFor={templateIdCampos.certificacoes}>
                  Certificações
                </label>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        ...
                        inverterValoresView(templateIdCampos.certificacoes);
                    }}
                  >
                    {props.getDadosSalvos?.objectViewComponentes
                      ?.certificacoes ? (
                      <textarea
                        ...
                        className="size-full resize-none w-full text-left max-h-48 min-h-48"
                        value={tratarValueInput(
                          props.getDadosSalvos?.objectParameter?.certificacoes,
                        )}
                        onChange={(event) => {
                          salvaValores(
                            templateIdCampos.certificacoes,
                            event.target.value,
                          );
                        }}
                        onClick={() =>
                          salvarDadosInputAtual(templateIdCampos.certificacoes)
                        }
    ...
    if (camposRetorno) {
      camposRetorno.push(
        <div
          className={`flex items-center ${props.parteAtualPreenchimento === 1 ? "justify-end" : "justify-between"}`}
          key={1}
        >
          {props.parteAtualPreenchimento > 1 && (
            <button
              onClick={() => {
                props.setParteAtualPreenchimento(
                  props.parteAtualPreenchimento - 1,
                );
              }}
            >
